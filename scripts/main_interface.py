...
def head():
    ...
    with gr.Blocks() as demo:
        ...
        class_corr = tmp[-12]["name"]
        tmp_lst = {class_corr: create_component(tmp[-12])}
        ...
        lst_headers = ['1 уровень', '2 уровень', 'Значения']

        btn = gr.Button("Submit")
        btn.click(# pylint: disable=no-member
            fn=feature_class_corr,# pylint: disable=no-member
            inputs=[comp[1] for comp in tmp_lst.values()],# pylint: disable=no-member
            outputs=gr.DataFrame(headers=lst_headers)# pylint: disable=no-member
        )# pylint: disable=no-member

        mean_cap_diam = tmp[-11]["name"]
        tmp_lst = {mean_cap_diam: create_component(tmp[-11])}
        ...
        tmp_lst = {class_begin: create_component(tmp[-10]), class_end: create_component(tmp[-9])}
        ...
        seas = tmp[-6]["name"]
        tmp_lst = {begin: create_component(tmp[-8]), end: create_component(tmp[-7]),
                   seas: create_component(tmp[-6])}
        ...
        box = tmp[-5]["name"]
        tmp_lst = {box: create_component(tmp[-5])}
        ...
        numeric = tmp[-3]["name"]
        cat = tmp[-2]["name"]
        tmp_lst = {numeric: create_component(tmp[-3]), cat: create_component(tmp[-2])}
        ...
        stem_width = tmp[-1]["name"]
        tmp_lst = {stem_width: create_component(tmp[-1])}
